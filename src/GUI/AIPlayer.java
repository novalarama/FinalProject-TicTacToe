...
public abstract class AIPlayer {
    protected int ROWS = 3; // Replace GameMain.ROWS with the actual value of ROWS
    ...
    private int evaluateLine(Cell c1, Cell c2, Cell c3, Seed seed) {
        ...
        // Count the number of seeds for the player and opponent
        int playerCount = 0, opponentCount = 0;
        Seed opponent = (seed == Seed.CROSS) ? Seed.NOUGHT : Seed.CROSS;
    
        if (c1.content == seed) playerCount++;
        if (c2.content == seed) playerCount++;
        if (c3.content == seed) playerCount++;
        if (c1.content == opponent) opponentCount++;
        if (c2.content == opponent) opponentCount++;
        if (c3.content == opponent) opponentCount++;
